...
window.addEventListener('resize', () =>
{
    ...
    sizes.pixeRatio=Math.min(window.devicePixelRatio,2)
...
const createFireworks=(count,position,size,texture,radius,color)=>{
 let positionsArray=new Float32Array(count *3)
 let sizesArray=new Float32Array(count)
 let timeMultiplier=new Float32Array(count)
 ...
 geometry.setAttribute("aTimeMultuplier",new THREE.Float32BufferAttribute(timeMultiplier,1))
...
createFireworks(100,new THREE.Vector3(),0.5,textures[7],1,new THREE.Color("#8affff"))
const createRandomFireWork=()=>{
    const count=Math.round(1000 +Math.random()*1000)
    const position=new THREE.Vector3(
        (Math.random()-0.5)*2,
        (Math.random()),
        (Math.random()-0.5 )*2
